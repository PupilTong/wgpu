...
- `wgpu`'s MSRV is now **1.88**, up from 1.87. `wgpu-core`, `wgpu-hal`, `wgpu-types` and `naga` are unchanged at 1.87. The Node-API bindings the `webgpu` backend uses on WASI depend on `napi-sys`, which declares 1.88, and Cargo checks `rust-version` across the whole resolved graph — a dependency declared only for WASI still counts on every target, so the floor applies wherever `webgpu` (a default feature) is enabled.

#### WebGPU

- The `web` feature now works on `wasm32-wasip1` and `wasm32-wasip1-threads`, where it reaches JavaScript through Node-API (napi-rs / emnapi) instead of wasm-bindgen. Which route `web` takes is decided by the target, not by a second feature, because on each target exactly one of the two can work: a WASI module cannot use wasm-bindgen at all — its bindings are satisfied by glue that `wasm-bindgen-cli` only emits for `wasm32-unknown-unknown`, and on WASI wasm-bindgen compiles them to stubs that panic, so such a build loads and then aborts on its first JavaScript call. The bindings live in the new `napi-rs-webgpu` crate, which declares WebGPU's IDL directly against Node-API, cut to exactly what `wgpu` reaches — 88 interfaces, 364 members, 32 string enums, proved minimal by emptying the surface and adding back only what the compiler asks for; a WASI build resolves neither `wasm-bindgen`, `js-sys` nor `web-sys`, and CI checks that. The addon installs its `napi_env` once per thread with `napi_rs_webgpu::install`, and the `GPU` object with `napi_rs_webgpu::install_gpu` where the host puts none on a global (as the `webgpu` npm package does not). The `webgl` backend is refused on WASI: WebGL2 is driven through wasm-bindgen and has no Node-API counterpart, so `wgpu-hal` leads the build failure with a `compile_error!` saying so. `fragile-send-sync-non-atomic-wasm` is refused alongside `web` on WASI for the same reason it always was — `wasm32-wasip1-threads` has real threads while reporting no `atomics` target feature, and a Node-API value belongs to the thread that owns its environment. Nothing changes for any other target. `napi-rs-webgpu/harness` is the acceptance test: an addon built for `wasm32-wasip1-threads`, loaded in Node with `@napi-rs/wasm-runtime` and handed the `GPU` from the `webgpu` npm package, which renders a triangle over half a 64×64 texture and checks the pixels that come back.
