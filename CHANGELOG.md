...
- New `napi-web` feature: the WebGPU backend can reach JavaScript through Node-API (napi-rs / emnapi) instead of wasm-bindgen, which makes `wasm32-wasip1` and `wasm32-wasip1-threads` usable targets. A WASI module cannot use wasm-bindgen at all — its bindings are satisfied by glue that `wasm-bindgen-cli` only emits for `wasm32-unknown-unknown`, and on WASI wasm-bindgen compiles them to stubs that panic, so such a build loads and then aborts on its first JavaScript call. With this feature the same WebGPU objects are driven over Node-API, which is what a napi-rs addon (`@napi-rs/wasm-runtime` + emnapi) has available. The bindings live in the new `napi-rs-webgpu` crate, which declares WebGPU's IDL — 94 interfaces, 437 members, 32 string enums — directly against Node-API; a WASI build resolves neither `wasm-bindgen`, `js-sys` nor `web-sys`, and CI checks that. The addon installs its `napi_env` once per thread with `napi_rs_webgpu::install`, and the `GPU` object with `napi_rs_webgpu::install_gpu` where the host puts none on a global (as the `webgpu` npm package does not). Nothing changes for any other target.
