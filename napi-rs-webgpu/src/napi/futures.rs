...
//! functions created by [`crate::napi::closure`].
//!
...
//!   [`crate::napi::value::JsValue`] handles converted by [`crate::napi::convert::FromJs`].
...
use crate::napi::closure::{stateless_function, Closure};
use crate::napi::convert::FromJs;
use crate::napi::env;
use crate::napi::value::JsValue;
use crate::support::Promise;
...
impl<T: FromJs + 'static> JsFuture<T> {
    ...
    fn attach(thenable: &JsValue) -> Self {
        ...
        let attached = crate::napi::rt::call_method(
            thenable,
            c"then",
            &[
        ...
        crate::napi::rt::unwrap_js(attached, "Promise.then");
...
fn settle<T>(state: &Rc<RefCell<Settlement<T>>>, result: Result<T, JsValue>) {
    let waker = {
        ...
        if state.result.is_some() {
            log::error!("napi-rs-webgpu: a promise settled twice");
            return;
        }
...
fn schedule_drain() {
    ...
    if !env::is_installed() {
        ...
        log::error!(
            "napi-rs-webgpu: spawn_local before napi_rs_webgpu::install(env) — \
    ...
    let queue_microtask = crate::napi::rt::global(c"queueMicrotask").unwrap_or(JsValue::UNDEFINED);
    let scheduled = if queue_microtask.is_function() {
        crate::napi::rt::call(&queue_microtask, &JsValue::UNDEFINED, &[drain])
    } else {
        crate::napi::rt::global(c"Promise")
            .and_then(|promise| crate::napi::rt::call_method(&promise, c"resolve", &[]))
            .and_then(|resolved| crate::napi::rt::call_method(&resolved, c"then", &[drain]))
    };
    if let Err(error) = scheduled {
        DRAIN_SCHEDULED.with(|flag| flag.set(false));
        log::error!("napi-rs-webgpu: could not schedule the task queue: {error}");
    }
}
