...
pub unsafe fn install(env: sys::napi_env) {
    assert!(
        !env.is_null(),
        "napi-rs-webgpu: install() needs a live napi_env, got null"
    );
...
pub(crate) fn env() -> sys::napi_env {
    try_env().expect(
        "napi-rs-webgpu: no Node-API environment installed on this thread — \
         call napi_rs_webgpu::install(env) from the addon's module init (or from a \
...
pub(crate) fn scope<R>(f: impl FnOnce(sys::napi_env) -> Result<R, JsValue>) -> Result<R, JsValue> {
    ...
    if status != sys::Status::napi_ok {
        return Err(JsValue::from_str(
            "napi-rs-webgpu: napi_open_handle_scope failed",
        ));
    }
    let result = f(env);
    ...
    debug_assert_eq!(
        close,
        sys::Status::napi_ok,
        "napi-rs-webgpu: napi_close_handle_scope failed"
    );
    result
}
...
pub(crate) fn check(status: sys::napi_status, operation: &str) -> Result<(), JsValue> {
    ...
    Err(JsValue::from_string(alloc::format!(
        "napi-rs-webgpu: {operation} failed with Node-API status {status}"
    )))
}
