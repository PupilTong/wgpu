...
pub fn global_this() -> Result<JsValue, JsValue> {
    env::scope(|env| {
        ...
        unsafe {
            let mut out = ptr::null_mut();
            env::check(sys::napi_get_global(env, &mut out), "napi_get_global")?;
            Ok(JsValue::from_napi(env, out))
        }
    })
}
...
/// The number of elements in an array-like value.
pub fn array_length(value: &JsValue) -> u32 {
    env::scope(|env| {
        // SAFETY: inside a handle scope on `env`.
        unsafe {
            let array = value.to_napi(env)?;
...
/// Writes `target[index] = value`.
pub fn set_index(target: &JsValue, index: u32, value: &JsValue) -> Result<(), JsValue> {
    env::scope(|env| {
        // SAFETY: inside a handle scope on `env`.
        unsafe {
            let object = target.to_napi(env)?;
            let value = value.to_napi(env)?;
            env::check(
                sys::napi_set_element(env, object, index, value),
                "napi_set_element",
            )
        }
    })
...
/// Reads `target[key]` with a computed key.
pub fn get_dynamic(target: &JsValue, key: &JsValue) -> Result<JsValue, JsValue> {
    env::scope(|env| {
        // SAFETY: inside a handle scope on `env`.
        unsafe {
            let object = target.to_napi(env)?;
...
/// Writes `target[key] = value` with a computed key.
pub fn set_dynamic(target: &JsValue, key: &JsValue, value: &JsValue) -> Result<(), JsValue> {
    env::scope(|env| {
        // SAFETY: inside a handle scope on `env`.
        unsafe {
            let object = target.to_napi(env)?;
            ...
            let value = value.to_napi(env)?;
            env::check(
                sys::napi_set_property(env, object, key, value),
                "napi_set_property",
            )
        }
    })
...
pub fn unwrap_js<T>(result: Result<T, JsValue>, operation: &str) -> T {
    match result {
        Ok(value) => value,
        Err(error) => {
            log::error!("wgpu-napi-web: {operation} threw: {error}");
            panic!("wgpu-napi-web: {operation} threw: {error}");
        }
    }
}
...
pub fn cast<T: JsCast>(value: JsValue) -> T {
    crate::convert::cast(value)
}

/// The message of a JavaScript error value, for diagnostics.
