...
use crate::env;
use crate::value::{JsCast, JsError, JsValue};
                ...
                fn into_js_function(self: Box<Self>) -> JsValue {
                        ...
                        R: ClosureReturn + 'static,
                    {
                        ...
                        if status != sys::Status::napi_ok || data.is_null() {
                            log::error!("wgpu-napi-web: callback invoked without its closure");
                            return ptr::null_mut();
                        }
                        ...
                        match result.into_js_result() {
                            Ok(value) => match value.to_napi(env) {
                                Ok(raw) => raw,
                                Err(error) => {
                                    log::error!("wgpu-napi-web: callback result unusable: {error}");
                                    ptr::null_mut()
                                }
                            },
                            Err(error) => {
                ...
                fn into_js_function_once(self) -> JsValue {
                    ...
                    let boxed: Box<dyn FnMut($($argument),*) -> R> = Box::new(
                        move |$($binding: $argument),*| {
                            let closure = closure
                                .take()
                                .expect("wgpu-napi-web: a `Closure::once` was called twice");
                            closure($($binding),*)
                        },
                    );
...
unsafe fn create_function(
    ...
    crate::rt::unwrap_js(created, "creating a JavaScript function for a Rust closure")
}
...
unsafe fn throw(env: sys::napi_env, error: &JsValue) {
    ...
    debug_assert_eq!(
        status,
        sys::Status::napi_ok,
        "wgpu-napi-web: napi_throw failed"
    );
}

/// Creates a JavaScript function calling a plain Rust function, with no state to
/// free. Used for the microtask drain in [`crate::futures`].
pub(crate) fn stateless_function(trampoline: sys::napi_callback) -> JsValue {
    ...
    unsafe {
        ...
        crate::rt::unwrap_js(created, "creating a JavaScript function for the task queue")
    }
}
