    ...
    unsafe fn value(&self) -> Result<sys::napi_value, JsValue> {
        ...
        if out.is_null() {
            ...
            return Err(JsValue::from_str(
                "wgpu-napi-web: JavaScript reference is no longer valid",
            ));
        }
        Ok(out)
    }
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            debug_assert_eq!(
                status,
                sys::Status::napi_ok,
                "wgpu-napi-web: napi_delete_reference failed"
            );
        }
    }
}
    ...
    pub(crate) unsafe fn to_napi(&self, env: sys::napi_env) -> Result<sys::napi_value, JsValue> {
        ...
        match &self.0 {
            ...
            Repr::Handle(handle) => {
                if handle.env != env {
                    return Err(JsValue::from_str(
                        "wgpu-napi-web: a JavaScript value from another Node-API \
        ...
        env::scope(|env| {
            ...
            unsafe {
                ...
                read_string(env, out).ok_or_else(|| {
                    JsValue::from_str("wgpu-napi-web: could not read coerced string")
                })
            }
        })
        .ok()
...
/// Any type that is representable as a [`JsValue`], the bound generic JS
/// containers use. Mirrors `wasm_bindgen::JsGeneric`.
pub trait JsGeneric: JsCast {}

impl<T: JsCast> JsGeneric for T {}

/// Marker for what a value resolves to when awaited, mirroring
/// `wasm_bindgen::sys::Promising`: for a promise it is the resolution type, for
/// everything else the type itself.
pub trait Promising {
    /// The type this value resolves to.
    type Resolution;
}
...
/// `unwrap`/`expect` that report the JavaScript error, mirroring
/// `wasm_bindgen::UnwrapThrowExt`.
///
/// wasm-bindgen throws the error into JavaScript; there is no frame to throw into
/// from here, so these panic. Under `panic = "abort"` — which is every WASI target
/// Rust ships — that ends the module, exactly as an uncaught `throw` from a
/// wasm-bindgen import would.
pub trait UnwrapThrowExt<T>: Sized {
    /// Unwraps, panicking with the JavaScript error on failure.
    fn unwrap_throw(self) -> T {
        self.expect_throw("`unwrap_throw` failed")
    }

    /// Unwraps, panicking with `message` and the JavaScript error on failure.
